...
std::vector<float> apply_iir_filtter(const std::vector<float>& input_signal, // Input signal
                                                 const std::vector<float>& feedforward_coefficients, // Feedforward coefficients
                                                 const std::vector<float>& feedback_coefficients, // Feedback coefficients
                                                 size_t feedforward_order,                     // Feedforward order
                                                 size_t feedback_order) {                   // Feedback order
    // Output signal y, initialized to zero
    std::vector<float> output_signal(input_signal.size(), 0.0);
...
void* process_iir_segment(void* arg) {
    ...
    for (size_t n = data->start_index; n < data->end_index; ++n) {
       ...
       // cout<<result.size()<<'\n';
        ...
        //   //  cout<<n<<" "<<k<<" "<<'\n';
        //     if (n >= k &&( n-k)<result.size()) {
        //         sum_feedback += feedback_coefficients[k] * result[n - k];
        //     }
        // }

       // cout<<data->start_index <<"   "<<data->end_index<<"  "<<n<<" "<<n - data->start_index<<'\n';
        result[n - data->start_index] = sum_feedforward;
    }
 ...
 size_t feedback_order = feedback_coefficients.size();
   ...
   for (size_t i = 0; i < num_threads; ++i) {
    ...
    for (size_t n = 0; n < input.size(); ++n){
      ...
      for (size_t i = 1; i <= feedback_order; ++i) {
            if (n >= i) {
                output[n] += feedback_coefficients[i] * output[n - i];
            }
        }
